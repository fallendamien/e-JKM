...
addshim:function(){
	...
	this.$shimobj=$j("#outlineiframeshim")
...
alignmenu:function(e, outlinemenu_pos){
	...
	var menuleft=($j(window).width()-(outlinemenu.offsetx-$j(document).scrollLeft())>outlinemenu.actualwidth)? outlinemenu.offsetx : outlinemenu.offsetx-outlinemenu.actualwidth+outlinemenu.anchorwidth //get x coord of menu
	var menutop=($j(window).height()-(outlinemenu.offsety-$j(document).scrollTop()+outlinemenu.anchorheight)>outlinemenu.actualheight)? outlinemenu.offsety+outlinemenu.anchorheight : outlinemenu.offsety-outlinemenu.actualheight //get y coord of menu
...
render:function($){
	for (var i=0, labels=this.outlinemenulabels[i]; i<this.outlinemenulabels.length; i++, labels=this.outlinemenulabels[i]){
		this.outlinemenus.push({$anchorobj:$j("#"+labels[0]), $menuobj:$j("#"+labels[1]), $menuinner:$j("#"+labels[1]).children('ul:first-child'), revealtype:labels[2]})
		...
		outlinemenu.$anchorobj.bind("mouseleave", function(e){
				...
				if (e.relatedTarget!=$menu.get(0) && $j(e.relatedTarget).parents("#"+$menu.get(0).id).length==0){ //check that mouse hasn't moved into menu object
