	...
	for (var i=0; i<Frm.elements.length; i++) {						// Loops through all the form's elements		
		if (Frm.elements[i].getAttribute(fv['code'])) {				// Gets the validator attribute, if exists thus starting the validation
			...
			switch (params[0]) {									// Calls appropriate validation function based on type				
				...
				case 'timePKM'		: if (validateTimePKM(validateObj)) testOk = true; break;
				case 'timenoam'	: if (validateTime2(validateObj)) testOk = true; break;
				...
				case 'checknumber'	: if (validateNumberAyu(validateObj, params[1], params[2], params[3])) testOk = true; break;
				case 'poskod'	: if (validatePoskod(validateObj, params[1], params[2], params[3])) testOk = true; break;
				case 'checkrumah'	: if (validateNoRumah(validateObj, params[1], params[2], params[3])) testOk = true; break;
				case 'checkbimbit'	: if (validateNoBimbit(validateObj, params[1], params[2], params[3])) testOk = true; break;
				case 'checkic'	: if (validateIC(validateObj, params[1], params[2], params[3])) testOk = true; break;
				case 'myTime'  : if (validateTimeValidation(validateObj)) testOk = true; break;//fatimah
				case 'unitUmur'  : if (validateAgeUnit(validateObj)) testOk = true; break; //nad
				
				case 'calculateMax'	: if (validateMaxUlasan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxKesihatan': if (validateMaxKesihatan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxTidakupaya': if (validateMaxTidakupaya(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxPesekolahan': if (validateMaxPesekolahan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxPekerjaan': if (validateMaxPekerjaan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxKemahiran': if (validateMaxKemahiran(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxTingkahLaku': if (validateMaxTingkahLaku(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxHubungan': if (validateMaxHubungan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxPegaulan': if (validateMaxPegaulan(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxRekodsalah': if (validateMaxRekodsalah(validateObj)) testOk = true; break;//pkm
				case 'calculateMaxHalLain': if (validateMaxHalLain(validateObj)) testOk = true; break;//pkm
...
/*
 *Create: 21 Oktober
 *Remark: To validate masa
 */
/***************************************************************************/
function validateMasa(formObj) {
 ...
 if (!regex.test(formObj.value)) {
  return errorProcess2(formObj,1,1,"Sila masukkan masa yang sah.");
  }
 return true;
 }
/***************************************************************************/
...
/***************************************************************************/
function validateNumberAyu(formObj,type, lb, ub) {
	...
	if (formObj.value.length < parseInt(lb) )	{
		return errorProcess2(formObj,1,1,'Please enter a number atleast ' + lb + 'characters');
		}
...
/***************************************************************************/
function validatePoskod(formObj,type, lb, ub) {
	...
	if (formObj.value=='00000'){
		return errorProcess2(formObj,1,1,'Please enter a number atleast ' + lb + 'characters');
		}

	var secondNum = (formObj.value).substring(1,2);
	//alert (secondNum);
...
/***************************************************************************/
function validateNoRumah(formObj,type, lb, ub) {
	var objName = formatName(formObj);
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
	...
	//alert(formObj.value);
	if ((formObj.value=='0000000')||(formObj.value=='00000000')||(formObj.value=='000000000')||(formObj.value=='0000000000')){		
		return errorProcess2(formObj,1,1,'Please enter a number atleast ' + lb + 'characters');
		}
...
/***************************************************************************/
function validateNoBimbit(formObj,type, lb, ub) {
	var objName = formatName(formObj);
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
...
/***************************************************************************/
function validateIC(formObj,type, lb, ub) {
	var objName = formatName(formObj);
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
...
function validateSelect(formObj) {
	...
	if (formObj.selectedIndex == 0) {
		...
		return errorProcess2(formObj,0,1,"Sila pilih "+objName); // Nora (20090910)
		}
	return true;
	}
...
function validateTime(formObj)	{
	...
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
	//alert('dfdsf');
	phoneReg = "^[0-12]:[0-59]([AP]M)?$";
	var regex = new RegExp(phoneReg);
	if (!regex.test(formObj.value)) {
		return errorProcess2(formObj,1,1,"Please enter a valid time.");
		}
	return true;
	}
	
function validateTimePKM(formObj)	{
	var objName = formatName(formObj);	
	if (fv['blank'] && checkBlank(formObj))
		{ fv['blank']=false; return true; }
	//alert('dfdsf');
	phoneReg = "^[0-12]:[0-59]([AP]M)?$";
	var regex = new RegExp(phoneReg);
	if (!regex.test(formObj.value)) {
	...
	function validateTime2(formObj)	{
	var objName = formatName(formObj);	
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
	//alert('dfdsf');
	phoneReg = "^(\d{1,2}):(\d{1,2})$";
	//var regex = new RegExp(phoneReg);
	...
	//if (!regex.test(formObj.value)) {
	if(regs = formObj.value.match(phoneReg)) {

		if(regs[1] < 1 || regs[1] > 12) {
          return errorProcess2(formObj,1,1,"Please enter a valid time.");
        } else if(regs[2] < 1 || regs[2] > 59) {
          return errorProcess2(formObj,1,1,"Please enter a valid time.");
        } 
...
function errorProcess2(o, sel, foc, error) {
	...
	if (fv['groupError']) { addError(o); ret = true; }
		...
		var alertMsg = (o.getAttribute(fv['emsg'])) ? o.getAttribute(fv['emsg']) : error;
		//var ja = jAlert(alertMsg,'Perhatian');
		alert(alertMsg);
		if (sel) o.select();
		if (foc) o.focus();
		}
...
function validateLUHN(ccString) {
	...
	/***************************************************************************/
function validateAgeUnit(formObj) {

 var objName = formatName(formObj);

 if (fv['bok'] && checkBlank(formObj))
  { fv['bok']=false; return true; }
 time = "TAHUN|BULAN|HARI";
 var regex = new RegExp(time);
 if (!regex.test(formObj.value)) {
  return errorProcess2(formObj,1,1,"Sila masukkan unit umur yang sah.");
  }
 return true;
 }

/***************************************************************************/
function validateTimeValidation(formObj)	{
	var objName = formatName(formObj);	
	if (fv['bok'] && checkBlank(formObj))
		{ fv['bok']=false; return true; }
	...
	var b = formObj.value;
	 ...
	 time = "01|02|03|04|05|06|07|08|09|10|11|12:[0-59]";
 	 var regex = new RegExp(time);
 	 if (!regex.test(formObj.value)) {
  	 return errorProcess2(formObj,1,1,"Sila masukkan masa yang sah.");
  	 }	
...
/***************************************************************************/
function validateMaxUlasan(formObj) {
if(document.main.pandangan.value.length > 4000) {
   alert('Data ULASAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.pandangan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
function validateMaxKesihatan(formObj) {
 if(document.main.kesihatan.value.length > 4000) {
   alert('Data KESIHATAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.kesihatan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 function validateMaxTidakupaya(formObj) {
if(document.main.tidakupaya.value.length > 4000) {
   alert('Data KETIDAKUPAYAAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.tidakupaya.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 function validateMaxPesekolahan(formObj) {
if(document.main.pesekolahan.value.length > 4000) {
   alert('Data PERSEKOLAHAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.pesekolahan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxPekerjaan(formObj) {
if(document.main.pekerjaan.value.length > 4000) {
   alert('Data PEKERJAAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.pekerjaan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxKemahiran(formObj) {
if(document.main.kemahiran.value.length > 4000) {
   alert('Data KEMAHIRAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.kemahiran.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxTingkahLaku(formObj) {
if(document.main.tingkahlaku.value.length > 4000) {
   alert('Data TINGKAHLAKU melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.tingkahlaku.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxHubungan(formObj) {
 if(document.main.hubungan.value.length > 4000) {
   alert('Data HUBUNGAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.hubungan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxPegaulan(formObj) {
if(document.main.pegaulan.value.length > 4000) {
   alert('Data PERGAULAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.pegaulan.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxRekodsalah(formObj) {
if(document.main.rekodsalah.value.length > 4000) {
   alert('Data REKOD KESALAHAN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.rekodsalah.value.length - 4000)+ ' perkataan ');
   return false;
...
/***************************************************************************/
 ...
 function validateMaxHalLain(formObj) {
if(document.main.hallain.value.length > 4000) {
   alert('Data HAL-HAL LAIN melebihi had maksima! Sila kurangkan sebanyak '+
    (document.main.hallain.value.length - 4000)+ ' perkataan ');
   return false;
